...
class EC(object):
    ...
    def add(self, p1, p2):
        ...
        if p1.x == p2.x:
            ...
            l = (3 * p1.x * p1.x + self.a) * modinv(2 * p1.y, self.q) % self.q
            pass
        else:
            l = (p2.y - p1.y) * modinv(p2.x - p1.x, self.q) % self.q
            pass
...
class MenezesVanstone(object):
    ...
    def hint(self, k):
        return self.ec.mul(g, k)

    def mask(self, pub, k):
        return self.ec.mul(pub, k)

    def enc(self, m1, m2, pub, k):
        hint = self.hint(k)
        mask = self.mask(pub, k)
        print "mask:", mask
        y1 = (m1 * mask.x) % ec.q
        y2 = (m2 * mask.y) % ec.q
        return hint, y1, y2

    def dec(self, hint, y1, y2, priv):
        c = ec.mul(hint, priv)
        print "c:", c
        m1 = y1 * modinv(c.x, ec.q) % ec.q
        m2 = y2 * modinv(c.y, ec.q) % ec.q
        return m1, m2
    pass
...
if __name__ == "__main__":
    ...
    print ec.numOfPoint()
    # print ec.findGenarator()
    # print ec.at(6)
    g = Coord(0, 3)
    print g, ec.order(g)

    mv = MenezesVanstone(ec, g)
    priv = 16
    pub = mv.gen(priv)
    print "pub:", pub
    hint, y1, y2 = mv.enc(7, 11, pub, 14)
    print "hint:", hint, "y1:", y1, "y2:", y2
    print mv.dec(hint, y1, y2, priv)
